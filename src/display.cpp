...
void configure_gpio() {
  const gpio_config_t input_conf = {
      .pin_bit_mask = (1ULL << LCD_PIN_RD), .mode = GPIO_MODE_INPUT, .pull_up_en = GPIO_PULLUP_ENABLE};
  ESP_ERROR_CHECK(gpio_config(&input_conf));

  gpio_config_t output_pin_config = {.pin_bit_mask = (1ULL << LCD_PIN_POWER) | (1ULL << LCD_PIN_BK_LIGHT),
...
void configure_lcd() {
  ...
  esp_lcd_panel_io_i80_config_t io_config = {.cs_gpio_num = LCD_PIN_CS,
  ...
  // Screen orientation/rotation defined here.
...
static void lvgl_tick_callback(void *arg) { lv_tick_inc(LVGL_TICK_PERIOD_MS); }
...
void lvgl_unlock() { xSemaphoreGiveRecursive(lvgl_mux); }
...
void create_display_timers() {
  ...
  const esp_timer_create_args_t lvgl_timer_args = {.callback = &lvgl_tick_callback, .name = "LVGL Timer"};
